...
async function resetPlayerPassword(playerId, playerName, btn) {
  ...
  try {
    ...
    // Clear authUid AND bump authEmailGen: if this player had already
    // migrated to real Firebase Auth, their old fake email
    // (id@padelx.local) is permanently claimed by that old Auth account —
    // client code can't delete someone else's Auth account, so simply
    // clearing authUid isn't enough, the next migration attempt would hit
    // "email-already-in-use" and silently fail (a real bug that locked a
    // player out). Bumping authEmailGen gives them a brand-new, never-used
    // fake email (id-g2@padelx.local, etc.) so the next login always
    // succeeds in creating a fresh Auth account, no matter how many times
    // the password has been reset before.
    await updateDoc(doc(db, "players", playerId), {
      passwordSalt: salt,
      passwordHash,
      authUid: null,
      authEmailGen: increment(1)
    });
